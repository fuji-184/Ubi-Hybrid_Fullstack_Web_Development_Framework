                    ...
                    "GET" => {
                        isi = match server::ROUTES.get(format_compact!("{}/get", path.strip_suffix("/").unwrap_or(path)).as_str()) {
                            ...
                            Some(handler) => {
                                ...
                                let url = format_compact!("{}/{}", path.strip_suffix("/").unwrap_or(path), req.method().to_lowercase());
 #[cfg(not(feature = "postgres"))]
            ...
            _ => {
                if !req.path().ends_with("/") {
                    match Frontend::get(&format_compact!("{}/index.html", req.path().strip_prefix("/").unwrap())) {
                        ...
                        None => {
                            let (path, _) = match_frontend_url(req.path()).unwrap();
                    ...
                    match Frontend::get(&format_compact!("{}index.html", path)) {
                        ...
                        None => {
                            ...
                            let (path, _) = match_frontend_url(req.path().strip_suffix("/").unwrap()).unwrap();
...
fn match_frontend_url(url: &str) -> Option<(&'static str, HashMap<String, String>)> {
    for (pattern, _) in server::FRONTEND_PARAMETERIZED_ROUTES.iter() {
        if let Some(params) = parameterized_url(pattern, url) {
...
fn main() -> io::Result<()> {
    ...
    may::config().set_pool_capacity(2000).set_stack_size(0x2000);
